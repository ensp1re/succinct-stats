...
import { Search, Star, AlertCircle, User, ArrowRight, Sparkles, ChevronDown, Shield, Info, Trophy, Zap } from "lucide-react"
...
export function UnifiedUserSearch() {
    ...
    const [stageTab, setStageTab] = useState("stage1")
    ...
    const [stage2ActiveTab, setStage2ActiveTab] = useState("search")
    const [stage2Username, setStage2Username] = useState("")
    const [stage2IsFocused, setStage2IsFocused] = useState(false)
    const [stage2IsSearching, setStage2IsSearching] = useState(false)
    const [stage2Error, setStage2Error] = useState("")
    const [stage2UserStats, setStage2UserStats] = useState(null)
    ...
    const handleSearch = () => {
        ...
        setTimeout(async () => {
            ...
            if (hasTwitter) {
                try {
                    // Normalize username: replace any "…" or "..." or similar with "..."
                    let normalizedUsername = username.trim()
                    // Replace unicode ellipsis or three dots (with or without spaces) with "..."
                    normalizedUsername = normalizedUsername.replace(/(\u2026|\. ?\. ?\.)/g, "...")
                    const result = await getEntryByUsername(normalizedUsername)
    ...
    // Stage 2 search handler
    const handleStage2Search = async () => {
        setStage2IsSearching(true)
        setStage2Error("")
        setStage2UserStats(null)
        try {
            const res = await fetch(`/api/prove-leaderboard?action=getByUsername&username=${encodeURIComponent(stage2Username)}`)
            if (!res.ok) {
                const data = await res.json()
                setStage2Error(data.error || "User not found")
            } else {
                const data = await res.json()
                setStage2UserStats(data.data)
                setStage2ActiveTab("profile")
            }
        } catch (e) {
            setStage2Error("Error searching user")
        } finally {
            setStage2IsSearching(false)
        }
    }
    const handleStage2KeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === "Enter") handleStage2Search()
    }

    function getGpuImage(type: string) {
        if (!type) return null;
        const color = type.toLowerCase();
        const validColors = ["pink", "purple", "orange", "blue", "green"];
        if (!validColors.includes(color)) return null;
        return `/images/team-${color}.png`;
    }
        ...
        <Card className="bg-white dark:bg-black border border-pink-300/50 dark:border-pink-900/50 shadow-lg shadow-pink-300/10 dark:shadow-pink-500/10 overflow-hidden">
            <CardHeader className="border-b border-pink-300/30 dark:border-pink-900/30 bg-gradient-to-r from-white to-pink-50/50 dark:from-black dark:to-pink-950/20">
                <Tabs value={activeStage} onValueChange={setActiveStage}>
                    <TabsList className="bg-white/20 dark:bg-black/20 border border-pink-300/30 dark:border-pink-900/30 w-full">
                        <TabsTrigger value="stage1" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white flex-1">
                            <Star className="h-4 w-4 mr-2" />
                            Stage 1
                        </TabsTrigger>
                        <TabsTrigger value="stage2" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white flex-1">
            ...
            <CardContent className="pt-6">
                {activeStage === "stage1" && (
                    <div className="relative">
                        ...
                        {/* Unified Search/Profile Tabs for Stage 1 */}
                        <Tabs value={activeTab} onValueChange={setActiveTab}>
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-lg font-mono text-pink-500"></h2>
                                                ...
                                                <div className="flex items-center space-x-4">
                                                    {userStats.avatar && (
                                                        <img
                                                            ...
                                                            alt={userStats.name ? String(userStats.name) : ''}
                ...
                {activeStage === "stage2" && (
                    <Tabs value={stage2ActiveTab} onValueChange={setStage2ActiveTab}>
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-lg font-mono text-pink-500"></h2>
                            <TabsList className="bg-white/20 dark:bg-black/20 border border-pink-300/30 dark:border-pink-900/30">
                                <TabsTrigger
                        ...
                        <TabsContent value="search" className="mt-0 space-y-6">
                            <div className="space-y-4">
                                <div className="bg-gradient-to-r from-pink-50/50 to-transparent dark:from-pink-950/20 dark:to-transparent p-4 rounded-lg border border-pink-300/30 dark:border-pink-900/30">
                                    <h3 className="text-pink-500 font-mono text-sm mb-3">
                                        SEARCH STAGE 2 USERS
                                    </h3>
                                    <div className="grid grid-cols-1 gap-4">
                                        <div className={`relative group ${stage2IsFocused ? "ring-2 ring-pink-500 rounded-md" : ""}`}>
                                            <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                                <User className={`h-5 w-5 ${stage2IsFocused ? "text-pink-500" : "text-gray-400 dark:text-gray-600"}`} />
                                            </div>
                                            <div className="absolute inset-y-0 left-10 flex items-center pointer-events-none">
                                                <div className="h-5 w-0.5 bg-gray-300 dark:bg-gray-700"></div>
                                            </div>
                                            <Input
                                                type="text"
                                                placeholder="Username (Twitter or Discord)..."
                                                value={stage2Username}
                                                onChange={(e) => setStage2Username(e.target.value)}
                                                onKeyDown={handleStage2KeyDown}
                                                onFocus={() => setStage2IsFocused(true)}
                                                onBlur={() => setStage2IsFocused(false)}
                                                className="bg-white dark:bg-black border border-pink-300/50 dark:border-pink-900/50 rounded-md pl-14 pr-4 py-2 text-gray-800 dark:text-white w-full focus:outline-none focus:ring-2 focus:ring-pink-500 transition-all duration-200"
                                            />
                                            {stage2Username && (
                                                <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                                                    <button
                                                        onClick={() => setStage2Username("")}
                                                        className="text-gray-400 hover:text-gray-600 dark:text-gray-600 dark:hover:text-gray-400"
                                                    >
                                                        <span className="sr-only">Clear input</span>
                                                        <svg
                                    ...
                                    <div className="mt-4 flex justify-center">
                                        <Button
                                            id="stage2-search-button"
                                            onClick={handleStage2Search}
                                            disabled={stage2IsSearching}
                                            className="w-full md:w-auto bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-700 hover:to-purple-600 text-white font-mono shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200"
                                        >
                                            {stage2IsSearching ? (
                                                <div className="flex items-center justify-center">
                                                    <div className="w-4 h-4 border-t-2 border-white rounded-full animate-spin mr-2"></div>
                            ...
                            {stage2Error && (
                                <div className="bg-pink-50 dark:bg-pink-950/20 border border-pink-300/50 dark:border-pink-900/50 rounded-md p-4 flex items-start animate-fadeIn">
                                    <AlertCircle className="text-pink-500 mr-2 h-5 w-5 mt-0.5 flex-shrink-0" />
                                    <div>
                                        <p className="text-pink-700 dark:text-pink-200">{stage2Error}</p>
                                        <p className="text-pink-600/70 dark:text-pink-300/70 text-sm mt-1">
                                            Try searching for another username or check if you've typed it correctly.
                                        </p>
                                    </div>
                            ...
                            {!stage2UserStats && !stage2Error && (
                                <div className="text-center py-10 text-gray-600 dark:text-gray-400 bg-gradient-to-br from-white to-purple-50/30 dark:from-black dark:to-purple-950/10 rounded-lg border border-purple-300/30 dark:border-purple-900/30">
                                    <div className="w-16 h-16 mx-auto mb-4 bg-purple-500/10 rounded-full flex items-center justify-center">
                                        <Trophy className="h-8 w-8 text-purple-400 dark:text-purple-500/50" />
                                    </div>
                                    <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">Find Stage 2 Users</h3>
                                    <p>Search by username to find Stage 2 points and rankings.</p>
                                    <p className="text-sm mt-2 text-gray-500 dark:text-gray-500">
                                        Enter a Twitter or Discord username to see their Stage 2 performance.
                                    </p>
                                </div>
                            )}
                        </TabsContent>
                        <TabsContent value="profile" className="mt-0 space-y-6">
                            {stage2UserStats && (
                                <>
                                    {(() => {
                                        const user: any = stage2UserStats;
                                        return (
                                            <>
                                                            ...
                                                            <h3 className="text-xl xs:text-2xl sm:text-2xl font-mono text-gray-800 dark:text-white mb-1 flex flex-wrap items-center gap-2">
                                                                ...
                                                                {user.gpu?.type && (
                                                                    <span className="ml-2 flex items-center gap-1">
                                                                        {getGpuImage(user.gpu.type) && (
                                                                            <img
                                                                                src={getGpuImage(user.gpu.type) as string}
                                                                                alt={`GPU ${user.gpu.type} badge`}
                                                                                className="w-6 h-6 rounded-full border border-gray-200 dark:border-gray-700 bg-white object-cover"
                                                                            />
                                                                        )}
                                                                        <span
                                                                            className="text-xs px-2 py-0.5 rounded-full font-mono border"
                                                                            style={{
                                                                                backgroundColor: user.gpu.color,
                                                                                color: "#fff", // White text for better contrast on any color
                                                                                borderColor: user.gpu.color,
                                                                                textShadow: "0 1px 2px rgba(0,0,0,0.25)" // subtle dark shadow for readability
                                                                            }}
                                                                        >
                                                                            {user.gpu.type.toUpperCase()}
                                                                        </span>
                                                                    </span>
                                                                )}
                                                            </h3>
                                                            <div className="flex items-center gap-2 mt-1">
                                                                {user.profileUrl && (
                                                                    <a
                                                                        ...
                                                                        target="_blank"
                                                                        rel="noopener noreferrer"
                                                                        className="text-xs text-pink-500 hover:underline font-mono"
                                                                    >
                                                                        View Profile
                                                                    </a>
                                                                )}
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                                    <div className="bg-white/50 dark:bg-black/50 p-4 rounded-lg border border-pink-300/30 dark:border-pink-900/30 shadow-sm">
                                                        <div className="flex justify-between mb-2">
                                                            <h3 className="text-pink-600 dark:text-pink-400 font-mono">PROOFS</h3>
                                                            <div className="flex items-center">
                                                                <Zap className="text-pink-500 w-4 h-4 mr-1" />
                                                                <span className="text-gray-800 dark:text-white font-mono">{user.proofs}</span>
                                                            </div>
                                                        </div>
                                                        <Progress
                                                            value={Math.min((user.proofs / 50) * 100, 100)}
                                                            className="h-3 bg-pink-100 dark:bg-pink-900/30"
                                                        >
                                                            <div className="h-full bg-gradient-to-r from-pink-600 to-pink-400 rounded-full" />
                                                        </Progress>
                                                    </div>
                                                    <div className="bg-white/50 dark:bg-black/50 p-4 rounded-lg border border-pink-300/30 dark:border-pink-900/30 shadow-sm">
                                                        <div className="flex justify-between mb-2">
                                                            <h3 className="text-pink-600 dark:text-pink-400 font-mono">POINTS</h3>
                                                            <div className="flex items-center">
                                                                <Trophy className="text-pink-500 w-4 h-4 mr-1" />
                                                                <span className="text-gray-800 dark:text-white font-mono">{user.points}</span>
                                                            </div>
                                                        </div>
                                                        <Progress
                                                            value={Math.min((user.points / 5000) * 100, 100)}
                                                            className="h-3 bg-pink-100 dark:bg-pink-900/30"
                                                        >
                                                            <div className="h-full bg-gradient-to-r from-pink-600 to-pink-400 rounded-full" />
                                                        </Progress>
                            ...
                            {!stage2UserStats && (
                                <div className="text-center py-10 text-gray-600 dark:text-gray-400 bg-gradient-to-br from-white to-purple-50/30 dark:from-black dark:to-purple-950/10 rounded-lg border border-purple-300/30 dark:border-purple-900/30">
                                    ...
                                    <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">No Stage 2 User Found</h3>
                                    <p>No user with this username was found in Stage 2.</p>
                                    <Button
                                        onClick={() => setStage2ActiveTab("search")}
                                        className="mt-4 bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-700 hover:to-purple-600 text-white"
                                    >
                                        <Search className="mr-2 h-4 w-4" />
                                        Try Another Search
                                    </Button>
